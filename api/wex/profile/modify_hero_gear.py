...
@auth(strict=True)
@sanic_ext.validate(json=MCPValidation.ModifyHeroGear, query=MCPQueryValidation.MCPProfile0)
@compress.compress()
async def modify_hero_gear(request: types.BBProfileRequest, accountId: str,
                           body: MCPValidation.ModifyHeroGear,
    ...
    if request_body.get("bIsInPit"):
        ...
        if not hero_item["templateId"].startswith("Character:"):
            raise errors.com.epicgames.world_explorers.bad_request(errorMessage="Invalid hero item id")
    if request_body.get("gearHeroItemId") != "":
        sidekick_template_id = (await request.ctx.profile.get_item_by_guid(request_body.get("gearHeroItemId")))[
                "templateId"]
        ...
        await request.ctx.profile.change_item_attribute(request_body.get("gearHeroItemId"), "used_as_sidekick", True)
        await request.ctx.profile.change_item_attribute(request_body.get("gearHeroItemId"), "sidekick_item_id",
                                                        request_body.get("heroItemId"))
        if request_body.get("bIsInPit"):
            await request.ctx.profile.change_item_attribute(request_body.get("heroItemId"), "sidekick_item_id",
                                                            request_body.get("gearHeroItemId"),
                                                            ProfileType.MONSTERPIT)
            await request.ctx.profile.change_item_attribute(request_body.get("heroItemId"), "sidekick_template_id",
