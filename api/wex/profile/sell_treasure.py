...
@auth(strict=True)
@sanic_ext.validate(json=MCPValidation.SellTreasure, query=MCPQueryValidation.MCPProfile0)
@compress.compress()
async def sell_treasure(request: types.BBProfileRequest, accountId: str,
                        body: MCPValidation.SellTreasure,
    ...
    item_path = (await get_path_from_template_id(request_body.get("itemTemplateId")))
    try:
        value = (await load_datatable(
            item_path.replace("res/battle-breakers-data/WorldExplorers/", "").replace(".json", "").replace("\\", "/")))[
            0][
            "Properties"]["GoldValue"]
    except KeyError:
        raise errors.com.epicgames.world_explorers.bad_request(errorMessage="This item cannot be sold.")
    item_guid = (await request.ctx.profile.find_item_by_template_id(request_body.get("itemTemplateId")))
    if not item_guid:
    ...
    sell_quantity = request_body.get("quantity")
