...
async def version_probe(request: types.BBRequest) -> sanic.response.HTTPResponse:
    ...
    if request.conn_info.ctx.probe_count > 1 or (hasattr(request.ctx, "last_probe_time") and 0.5 >= (
            datetime.datetime.now(tz=datetime.timezone.utc) - request.ctx.last_probe_time).total_seconds() <= 1.5):
